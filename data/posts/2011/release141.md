...
There is far too much to pack into a short list, the full change list can be found [here](http://exist.svn.sourceforge.net/viewvc/exist/stable/eXist-1.4.x/?view=log) but, the highlights include:
...
3.  WebDAV – completely new implementation, based on the solid [Milton WebDAV Server library](http://milton.ettrema.com/). Enabled by default, but don't panic, the old version is still present should you need it!
4.  Lucene Full-Text Indexing – upgraded to version [2.9.2](https://lucene.apache.org/java/2_9_2/) for a performance boost, configurable analyzers, configurable parameters, better match highlighting and additional functions for working with Lucene document fields.
5.  XForms – The [betterFORM](http://www.betterform.de) server-side XForms 1.1 engine now ships with eXist-db and is enabled by default. The included *XSLTForms* XForms engine has been updated.
6.  EXPath – The [EXPath](http://expath.org/) projects HTTP Client XQuery module has been implemented as a first step towards EXPath portable XQuery.
